...
func TestPolicyContract_Builtins(t *testing.T) {
	...
	if ns.Allow(&adapter.CanonicalTool{Namespace: "beta", Name: "echo"}) {
...
func TestPolicyContract_NilFunc(t *testing.T) {
	var fn PolicyFunc
	if fn.Allow(&adapter.CanonicalTool{Name: "noop"}) {
...
func (s stubRegistry) Tools() []*adapter.CanonicalTool {
	return s.tools
}

func TestRegistryContract_BuilderUsesSnapshot(t *testing.T) {
	tools := []*adapter.CanonicalTool{
		{Name: "one", InputSchema: &adapter.JSONSchema{Type: "object"}},
	}
	...
	tools[0] = &adapter.CanonicalTool{Name: "changed", InputSchema: &adapter.JSONSchema{Type: "object"}}
	if ts.Count() != 1 {
