...
func (r *mockRegistry) Tools() []*adapter.CanonicalTool {
	return r.tools
}
...
func TestBuilder_FromTools(t *testing.T) {
	...
	t.Run("empty slice returns empty toolset", func(t *testing.T) {
		ts, err := NewBuilder("test").FromTools([]*adapter.CanonicalTool{}).Build()
		if err != nil {
			t.Fatalf("Build() error = %v", err)
		}
		if ts.Count() != 0 {
...
func TestBuilder_WithCategories(t *testing.T) {
	t.Run("filters to tools with any category", func(t *testing.T) {
		tools := []*adapter.CanonicalTool{
			{Name: "search", Namespace: "ns", Category: "query", InputSchema: &adapter.JSONSchema{Type: "object"}},
			{Name: "delete", Namespace: "ns", Category: "mutation", InputSchema: &adapter.JSONSchema{Type: "object"}},
			{Name: "find", Namespace: "ns", Category: "query", InputSchema: &adapter.JSONSchema{Type: "object"}},
		}
		ts, err := NewBuilder("test").
			FromTools(tools).
...
func TestBuilder_WithFilter(t *testing.T) {
	t.Run("custom FilterFunc applied", func(t *testing.T) {
		...
		ts, err := NewBuilder("test").
			FromTools(tools).
			WithFilter(func(t *adapter.CanonicalTool) bool {
				return t.Name[0] == 'a'
			}).
			Build()
		if err != nil {
...
func TestBuilder_Build_Success(t *testing.T) {
	t.Run("returns toolset and nil error on success", func(t *testing.T) {
		tools := []*adapter.CanonicalTool{makeTool("ns", "a", nil)}
		ts, err := NewBuilder("test").FromTools(tools).Build()
		if err != nil {
