...
func makeTool(namespace, name string, tags []string) *adapter.CanonicalTool {
	return &adapter.CanonicalTool{
		Namespace:   namespace,
		Name:        name,
		Tags:        tags,
...
func TestToolset_Filter(t *testing.T) {
	t.Run("filter returns new toolset instance", func(t *testing.T) {
		...
		filtered := ts.Filter(func(*adapter.CanonicalTool) bool { return true })
	...
	t.Run("original unchanged after filter", func(t *testing.T) {
		...
		_ = ts.Filter(func(t *adapter.CanonicalTool) bool {
			return t.Name == "a"
		})
	...
	t.Run("filter with always-false returns empty toolset", func(t *testing.T) {
		...
		filtered := ts.Filter(func(*adapter.CanonicalTool) bool { return false })
